...
public class Phase2Options {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!Phase2Options.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Phase2Options is not found in the empty JSON string", Phase2Options.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Phase2Options.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Phase2Options` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("Phase2DhGroupNumbers") != null && !jsonObj.get("Phase2DhGroupNumbers").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `Phase2DhGroupNumbers` to be an array in the JSON string but got `%s`", jsonObj.get("Phase2DhGroupNumbers").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("Phase2EncryptionAlgorithms") != null && !jsonObj.get("Phase2EncryptionAlgorithms").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `Phase2EncryptionAlgorithms` to be an array in the JSON string but got `%s`", jsonObj.get("Phase2EncryptionAlgorithms").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("Phase2IntegrityAlgorithms") != null && !jsonObj.get("Phase2IntegrityAlgorithms").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `Phase2IntegrityAlgorithms` to be an array in the JSON string but got `%s`", jsonObj.get("Phase2IntegrityAlgorithms").toString()));
      }
      if ((jsonObj.get("PreSharedKey") != null && !jsonObj.get("PreSharedKey").isJsonNull()) && !jsonObj.get("PreSharedKey").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `PreSharedKey` to be a primitive type in the JSON string but got `%s`", jsonObj.get("PreSharedKey").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       return (TypeAdapter<T>) new TypeAdapter<Phase2Options>() {
           @Override
