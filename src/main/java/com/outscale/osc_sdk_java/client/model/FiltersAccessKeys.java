...
public class FiltersAccessKeys {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!FiltersAccessKeys.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FiltersAccessKeys is not found in the empty JSON string", FiltersAccessKeys.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FiltersAccessKeys.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FiltersAccessKeys` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("AccessKeyIds") != null && !jsonObj.get("AccessKeyIds").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `AccessKeyIds` to be an array in the JSON string but got `%s`", jsonObj.get("AccessKeyIds").toString()));
      }
      // ensure the optional json data is an array if present
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       return (TypeAdapter<T>) new TypeAdapter<FiltersAccessKeys>() {
           @Override
           public void write(JsonWriter out, FiltersAccessKeys value) throws IOException {
