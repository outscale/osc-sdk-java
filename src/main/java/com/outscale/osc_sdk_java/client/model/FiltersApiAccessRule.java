...
public class FiltersApiAccessRule {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!FiltersApiAccessRule.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FiltersApiAccessRule is not found in the empty JSON string", FiltersApiAccessRule.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FiltersApiAccessRule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FiltersApiAccessRule` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("ApiAccessRuleIds") != null && !jsonObj.get("ApiAccessRuleIds").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `ApiAccessRuleIds` to be an array in the JSON string but got `%s`", jsonObj.get("ApiAccessRuleIds").toString()));
      }
      // ensure the optional json data is an array if present
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       return (TypeAdapter<T>) new TypeAdapter<FiltersApiAccessRule>() {
           @Override
           public void write(JsonWriter out, FiltersApiAccessRule value) throws IOException {
