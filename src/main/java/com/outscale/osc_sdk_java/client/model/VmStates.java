...
public class VmStates {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!VmStates.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in VmStates is not found in the empty JSON string", VmStates.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!VmStates.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VmStates` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("MaintenanceEvents") != null && !jsonObj.get("MaintenanceEvents").isJsonNull()) {
        ...
        if (jsonArraymaintenanceEvents != null) {
          ...
          if (!jsonObj.get("MaintenanceEvents").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `MaintenanceEvents` to be an array in the JSON string but got `%s`", jsonObj.get("MaintenanceEvents").toString()));
          }

          // validate the optional field `MaintenanceEvents` (array)
          for (int i = 0; i < jsonArraymaintenanceEvents.size(); i++) {
            MaintenanceEvent.validateJsonObject(jsonArraymaintenanceEvents.get(i).getAsJsonObject());
          };
        }
      }
      ...
      if ((jsonObj.get("VmState") != null && !jsonObj.get("VmState").isJsonNull()) && !jsonObj.get("VmState").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `VmState` to be a primitive type in the JSON string but got `%s`", jsonObj.get("VmState").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       return (TypeAdapter<T>) new TypeAdapter<VmStates>() {
           @Override
