...
public class ReadAdminPasswordResponse {
    ...
    public static void validateJsonObject(JsonObject jsonObj) throws IOException {
        if (jsonObj == null) {
            if (!ReadAdminPasswordResponse.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON object is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in ReadAdminPasswordResponse is not"
                                        + " found in the empty JSON string",
                                ReadAdminPasswordResponse.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ReadAdminPasswordResponse` properties. JSON: %s",
                                entry.getKey(), jsonObj.toString()));
            }
        }
        if ((jsonObj.get("AdminPassword") != null && !jsonObj.get("AdminPassword").isJsonNull())
                && !jsonObj.get("AdminPassword").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `AdminPassword` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("AdminPassword").toString()));
        }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ReadAdminPasswordResponse.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ReadAdminPasswordResponse' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<ReadAdminPasswordResponse>() {
                        @Override
                        public void write(JsonWriter out, ReadAdminPasswordResponse value)
                                throws IOException {
