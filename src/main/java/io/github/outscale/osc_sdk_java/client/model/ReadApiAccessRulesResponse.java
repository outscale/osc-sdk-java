...
public class ReadApiAccessRulesResponse {
    ...
    public static void validateJsonObject(JsonObject jsonObj) throws IOException {
        if (jsonObj == null) {
            if (!ReadApiAccessRulesResponse.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON object is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in ReadApiAccessRulesResponse is not"
                                        + " found in the empty JSON string",
                                ReadApiAccessRulesResponse.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ReadApiAccessRulesResponse` properties. JSON: %s",
        ...
        if (jsonObj.get("ApiAccessRules") != null && !jsonObj.get("ApiAccessRules").isJsonNull()) {
            ...
            if (jsonArrayapiAccessRules != null) {
                ...
                if (!jsonObj.get("ApiAccessRules").isJsonArray()) {
                    throw new IllegalArgumentException(
                            String.format(
                                    "Expected the field `ApiAccessRules` to be an array in the"
                                            + " JSON string but got `%s`",
                                    jsonObj.get("ApiAccessRules").toString()));
                }

                // validate the optional field `ApiAccessRules` (array)
                for (int i = 0; i < jsonArrayapiAccessRules.size(); i++) {
                    ApiAccessRule.validateJsonObject(
                            jsonArrayapiAccessRules.get(i).getAsJsonObject());
                }
                ;
            }
        }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ReadApiAccessRulesResponse.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ReadApiAccessRulesResponse' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<ReadApiAccessRulesResponse>() {
                        @Override
                        public void write(JsonWriter out, ReadApiAccessRulesResponse value)
                                throws IOException {
