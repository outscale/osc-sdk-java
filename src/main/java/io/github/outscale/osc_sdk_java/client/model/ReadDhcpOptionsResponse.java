...
import io.github.outscale.osc_sdk_java.client.model.DhcpOptionsSet;
...
public class ReadDhcpOptionsResponse {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ReadDhcpOptionsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ReadDhcpOptionsResponse is not found in the empty JSON string", ReadDhcpOptionsResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ReadDhcpOptionsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReadDhcpOptionsResponse` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("DhcpOptionsSets") != null && !jsonObj.get("DhcpOptionsSets").isJsonNull()) {
        ...
        if (jsonArraydhcpOptionsSets != null) {
          ...
          if (!jsonObj.get("DhcpOptionsSets").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `DhcpOptionsSets` to be an array in the JSON string but got `%s`", jsonObj.get("DhcpOptionsSets").toString()));
          }

          // validate the optional field `DhcpOptionsSets` (array)
          for (int i = 0; i < jsonArraydhcpOptionsSets.size(); i++) {
            DhcpOptionsSet.validateJsonObject(jsonArraydhcpOptionsSets.get(i).getAsJsonObject());
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ReadDhcpOptionsResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ReadDhcpOptionsResponse' and its subtypes
       }
       ...
       return (TypeAdapter<T>) new TypeAdapter<ReadDhcpOptionsResponse>() {
           @Override
           public void write(JsonWriter out, ReadDhcpOptionsResponse value) throws IOException {
